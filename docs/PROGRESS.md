...
| `pm-engine` — graphe LangGraph complet | Valide de bout en bout | Run complet autonome sur un vrai ticket Forgejo (`todo-app#12`, 2026-08-31) : analyse, decoupage en sous-taches paralleles, 3 vraies microVM Firecracker, `exec_in_workshop`, tests, boucle de correction, **PR de 12 fichiers / 941 lignes**, suspension et revue HITL enregistree. Le decoupage parallele produisait du travail redondant (deux implementations de la meme fonctionnalite) : cause identifiee le 2026-09-01 en relisant les 21 plans reels du checkpointer — le planificateur ne voyait PAS le depot. Les 20 plans multi-taches ont ete produits alors que `main` ne contenait qu'un `README.md` : chaque agent devait donc inventer son propre socle dans son propre Workshop. Le planificateur recoit desormais le contenu reel de la racine (`list_root_entries`, les 3 providers) et un depot vierge force une sous-tache unique |
