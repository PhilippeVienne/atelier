...
- Tableaux de bord Grafana custom : non nécessaire — **découvert après coup** en répondant à la question « a-t-on un dashboard ? » : l'image `grafana/otel-lgtm` retenue au §3 auto-provisionne déjà un dashboard « RED Metrics » (`/otel-lgtm/grafana-dashboard-red-metrics-classic.json` dans l'image), dont les requêtes PromQL correspondent EXACTEMENT aux noms de métriques produits par `crate::http_metrics` (`http_server_duration_milliseconds_{count,bucket}`). Vérifié empiriquement (traffic réel généré, requêtes du dashboard rejouées à la main contre Prometheus) : les panneaux Request Rate et Duration percentiles (p50/p95) fonctionnent tels quels. Seul le panneau Error Rate filtrait sur un nom d'attribut (`http.response.status_code`, convention OTel actuelle) différent de celui produit initialement (`http.status_code`, dépréciée) — corrigé en alignant `crate::http_metrics` sur le nom attendu, revérifié (un vrai `401` apparaît bien avec le label `http_response_status_code`).
