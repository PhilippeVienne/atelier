...
**Règle pour tout futur rôle parallèle** : n'écrire, dans l'état partagé, que des clés dont on est seul responsable à cet instant du graphe — jamais une clé « générique » (`phase`, `status`...) que plusieurs branches concurrentes pourraient toucher au même superstep, sauf à la déclarer `Annotated` avec un réducteur qui sait fusionner deux écritures (LangGraph refuse sinon deux écritures concurrentes différentes sur une même clé). `ReviewSecurity`/`ReviewOps` n'écrivent donc pas `"phase"`, contrairement aux autres nœuds — seul `ReviewGate`, point de convergence, la porte pour les deux.
