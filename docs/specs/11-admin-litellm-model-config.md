...
- **Fausse piste, corrigee apres re-verification methodique** : une premiere passe avait conclu que `POST /model/update` sans `api_key` EFFACE la cle existante, en se basant sur le fait que `GET /model/info` ne renvoie jamais `litellm_params.api_key`. **Erreur de methode** : `GET /model/info` ne renvoie ce champ dans AUCUN cas, meme juste apres une creation/modification qui vient de le poser — son absence de la reponse ne prouve donc RIEN sur ce qui est reellement stocke. Verifie autrement (test FONCTIONNEL, seul fiable ici) : creer un modele avec une vraie cle DeepSeek fonctionnelle, faire un appel `/v1/messages` reel (succes), `POST /model/update` SANS `api_key` (seul `model_name` change), refaire le meme appel `/v1/messages` — **succes, identique** : la cle est bien **preservee**, pas effacee. Confirme a l'inverse en fournissant une cle volontairement invalide sur `update` : l'appel suivant echoue bien en `401` — la fusion est donc reelle (litellm_params est fusionne champ par champ, pas remplace integralement comme suppose initialement). **Consequence : aucune copie de secours n'est necessaire cote `atelier-api-server`** — `update_model(api_key: None)` peut se contenter d'omettre le champ, LiteLLM se charge de preserver l'ancienne valeur. Le code de `crate::llm_budget::LlmBudgetClient::update_model` (implemente en 6.7.2, avant cette re-verification) faisait deja exactement ca — c'etait correct des le depart, seule cette note de spec (et une tentative d'implementation OpenBao ensuite abandonnee) etait erronee.
