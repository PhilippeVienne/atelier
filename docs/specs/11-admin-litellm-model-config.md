...
- **Découverte bloquante pour §4.1 (write-only), vérifiée empiriquement** : `GET /model/info` ne renvoie JAMAIS `litellm_params.api_key`, même immédiatement après un `/model/new` qui vient de l'enregistrer (champ absent de la réponse, testé avec une valeur en clair connue). Consequence directe : `POST /model/update` **remplace entièrement** `litellm_params` — un appel qui omet `api_key` (pensé comme « ne pas changer la clé ») l'**efface** cote LiteLLM (`api_key` devient absent, testé empiriquement, cluster de dev). Il n'existe **aucun moyen** pour `atelier-api-server` de « relire puis renvoyer » l'ancienne clé pour simuler une preservation, puisque LiteLLM lui-même ne la rend jamais. Le formulaire d'édition (`6.7.4`) ne peut donc PAS se contenter d'un champ `api_key` laissé vide pour « ne pas toucher a la cle » — deux options reelles, a trancher avant `6.7.4` :
  1. Exiger la ressaisie complete de la cle a CHAQUE modification (perd l'ergonomie « modifier juste l'alias/api_base sans toucher au secret », mais reste honnete et ne complique rien cote infra) ;
  2. `atelier-api-server` garde sa PROPRE copie chiffree de la cle (base Postgres deja utilisee par ce service) pour la renvoyer silencieusement lors d'un `update` qui ne fournit pas de nouvelle valeur — reintroduit exactement la duplication de source de verite ecartee en §4.2, a peser explicitement plutot qu'a decouvrir en cours d'implementation de `6.7.4`.
