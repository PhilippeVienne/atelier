...
3. **Propagation dans les MicroVMs** :
   - Lors de la préparation de l'environnement ou au premier boot, la CA racine est injectée dans `/usr/local/share/ca-certificates/atelier-ca.crt` puis `update-ca-certificates` est invoqué (ou append direct dans `/etc/ssl/certs/ca-certificates.crt`).
   - Tous les outils in-VM (`git clone`, `npm install`, `cargo`, `pip`, `curl`) reconnaissent immédiatement les miroirs internes sans aucun avertissement.
