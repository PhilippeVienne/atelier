...
3. **Intégration CLI Single-Node (`atelier server install --enable-gpu`, tâche 11.4, implémentée)** : `crates/cli/src/commands/server.rs::detect_nvidia_gpu` interroge réellement le pilote via `nvidia-smi` (NVML, pas seulement `ls /dev/nvidia*` — même principe que la vérification KVM déjà existante via `kvm_ioctls::Kvm::new()`) ET vérifie la présence de `nvidia-container-runtime` dans le `PATH` (sans lui, k3s/containerd ne peut exposer aucun GPU aux pods, quel que soit le pilote hôte). Intégré à `doctor` comme diagnostic non bloquant ("GPU NVIDIA (optionnel, --enable-gpu)"). Le flag `--enable-gpu` d'`install` rend ce même diagnostic BLOQUANT (échec explicite avant toute modification système si aucun GPU utilisable n'est détecté, plutôt qu'un chart installé avec `gpu.enabled: true` dont le `StatefulSet` vLLM resterait `Pending` indéfiniment) et positionne `gpu.enabled: true`/`gpu.resources.limits."nvidia.com/gpu": 1` dans les values Helm générées (`write_values_file`). **Vérifié en réel sur cette machine de développement** (GPU physique NVIDIA RTX 3060 présent) : `atelier server doctor` détecte correctement `NVIDIA GeForce RTX 3060, 8192 MiB` via `nvidia-smi`, et signale à raison l'absence de `nvidia-container-runtime` sur cette machine (non installé) comme avertissement non bloquant et actionnable. **Non vérifié** : `install --enable-gpu` de bout en bout (installerait réellement k3s sur cette machine hôte partagée, même limite déjà documentée pour la tâche 9.8) — la fonction de détection elle-même est vérifiée en réel, son câblage dans `install`/`write_values_file` est vérifié par lecture de code et compilation, pas par une exécution complète.
4. **`StatefulSet` vLLM non déployé/testé en conditions réelles d'inférence** : `nvidia-container-toolkit` absent de cette machine (confirmé ci-dessus) et `docker run --gpus all` y échoue explicitement (`could not select device driver`) — les installer serait un changement système invasif (modification du runtime Docker par défaut) hors du périmètre raisonnable de cette tâche sur une machine de dev partagée. Resterait de toute façon `Pending` sans device plugin Kubernetes `nvidia.com/gpu` (prérequis non couvert par ce chart, voir tâche 11.3) : seuls `helm lint`/`helm template` (rendu vérifié avec `gpu.enabled: true`) et `shellcheck` (script du Job d'enregistrement) le couvrent côté template ; la logique métier du Job (idempotence, forme du payload) est, elle, vérifiée en réel comme décrit en 11.3.
...
### 3.3. Outillage Déconnecté (Air-Gap Packaging) — tâche 11.5, **implémentée**

1. **Préfixage des Registres — correction par rapport à la première rédaction** : `.Values.global.imageRegistry` **n'existe pas** dans ce chart (vérifié : aucun template de `charts/atelier/templates/` ne le lit). Deux mécanismes bien réels, mais séparés :
   - Les 5 composants injectés par le CONTROLLER dans les pods Workshop (`net-proxy`/`identity-proxy`/`vm-supervisor`/`mcp-gateway`/`image-builder`) se reconfigurent globalement via `controller.env.ATELIER_COMPONENT_IMAGE_REGISTRY` (`crates/controller/src/reconcile.rs::component_image_ref`, déjà existant).
   - Tout le reste (`api-server`, `controller`, `dashboard`, `pm-engine`, `kvm-device-plugin`, et toutes les dépendances tierces — Postgres, Keycloak, Forgejo, OpenBao, LiteLLM, Redis, RustFS, observability, vLLM) n'a PAS de préfixe global : chaque section de `values.yaml` porte son propre `<composant>.image.repository`, à surcharger individuellement. Non fait dans cette tâche (chantier de chart distinct) : ajouter un véritable `global.imageRegistry` cohérent à toutes les sections serait une amélioration future légitime, découverte en implémentant celle-ci.
2. **Script d'Export/Import (`scripts/airgap-bundle.sh`)** — **correction** : `skopeo`/`docker save` abandonnés au profit de `crane pull`/`crane push` (déjà vendu et utilisé ailleurs dans ce dépôt — `deploy/dev/crane/crane`, `crates/image-builder` — aucune nouvelle dépendance externe). `export [--with-gpu] [output-dir]` télécharge chaque image en tarball individuel (`crane pull`) puis les archive en `atelier-images-<version>.tar.gz` (version lue depuis `charts/atelier/Chart.yaml`, `--with-gpu` inclut `vllm/vllm-openai` — plusieurs Go, exclu par défaut) ; `import --registry <registre>` extrait l'archive et rejoue chaque image (`crane push`), en re-taguant les 5 composants injectés par le controller en `dev` (même convention que `deploy/terraform/aws/mirror-images.sh`). Liste des images tenue à jour manuellement contre `values.yaml` — `mirror-images.sh` a pris du retard (omet `observability`/RustFS/`gpu`/`litellmVllmModelInit`, confond `postgres:16-alpine` — client `psql` des Jobs d'init — avec `pgvector/pgvector:pg16`, l'image réelle de la base) : liste corrigée dans ce nouveau script, `mirror-images.sh` non touché (hors périmètre de cette tâche).
   - **Piège trouvé et corrigé en testant réellement** : `do_import` déclarait `local work_dir` puis un `trap 'rm -rf "$work_dir"' EXIT` — le trap s'exécute après le RETOUR de la fonction, où `work_dir` (une variable `local`) est déjà hors de portée, provoquant `variable sans liaison` sous `set -u` (import réussi, mais message d'erreur final trompeur, `exit 1`). Corrigé en retirant `local` (variable de script, pas de fonction).
   - **Vérifié de bout en bout contre de vraies images et un vrai registre** : `crane pull ghcr.io/philippevienne/atelier-net-proxy:latest`/`curlimages/curl:8.11.0` réels, `export` produit une archive réelle (~45 Mo), `import --registry localhost:<port>` contre un vrai `registry:2` local confirme les deux images présentes (`GET /v2/_catalog`), `atelier-net-proxy` bien re-tagué `dev`. `shellcheck` (`koalaman/shellcheck:stable`) propre après correctif.
   - Commande d'import (identique à la première rédaction) :
     ```bash
