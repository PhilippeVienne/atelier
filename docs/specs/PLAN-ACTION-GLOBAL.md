...
* **Fichiers** : `services/pm-engine/pm_engine/{state,nodes,graph}.py`, `git_providers/base.py` (+ `forgejo.py`/`github.py`/`gitlab.py`)
  - [x] **5.6.1** : `BaseGitProvider.get_diff` (non abstraite, `None` par défaut) + implémentations Forgejo/GitHub/GitLab. *(Fait le 2026-09-02 : Forgejo `9.0.3+gitea-1.22.0` n'expose PAS de diff brut branche-à-branche (`.../compare/{base}...{head}.diff` répond 404, constaté contre l'instance de dev réelle) — contournement en listant les commits de la comparaison (JSON) puis en concaténant leur diff individuel (`/git/commits/{sha}.diff`, `200 text/plain`, confirmé). GitHub (`Accept: vnd.github.v3.diff`) et GitLab (`diffs[].diff`, sans l'en-tête `diff --git` qu'il a fallu synthétiser) vérifiés contre leurs vraies API publiques (`octocat/Hello-World`, `gitlab-org/gitlab-test`). Test réel `test_get_diff_reads_content_added_on_a_branch` contre l'instance Forgejo de dev, vert.)*
  - [x] **5.6.2** : Détection déterministe des chemins sensibles/infra (`SECURITY_SENSITIVE_PATTERNS`/`OPS_SENSITIVE_PATTERNS`, §4.2) — pas de LLM pour cette décision. *(Fait le 2026-09-02 : `nodes.diff_matches_any_pattern`, extraction des chemins via la ligne `diff --git a/... b/...` commune aux trois `get_diff` (5.6.1). Piège trouvé en écrivant les tests : `fnmatch` n'est pas un glob de chemin — `**/*.tf` exige un `/` réellement présent, donc ne matche PAS un fichier à la racine (`main.tf`) sans un repli explicite sur le motif sans son préfixe `**/` — couvert par `_path_matches` et testé dans les deux cas (imbriqué et racine).)*
  - [x] **5.6.3** : Nœud `ReviewArchitecture` (après `PlanParallelTasks`/`ExpandGreenfieldSpec`, avant `ProvisionWorkshop`), bouclage borné vers `PlanParallelTasks` (compteur dédié `architecture_review_attempts`). *(Fait le 2026-09-02 : bouclage via un nœud intermédiaire `ArchitectureReconsideration` — injecte les objections dans `analysis`, LangGraph route sur des noms de nœuds, pas sur un appel de fonction arbitraire. Nouveau modèle mock `atelier-review-test` (`deploy/dev/llm-proxy/config.yaml`) pour tester un vrai aller-retour LiteLLM sur le chemin `request_changes`, ConfigMap dev redéployée. Suite `pm-engine` verte (61 passed, 19 skipped avec la config d'environnement habituelle de cette session) ; deux tests pré-existants, sans rapport avec cette tâche, restent en échec quand on force leurs identifiants complets — voir `pieges.md` 2026-09-02, port-forward LiteLLM manquant sur le port 14000 puis ambiguïté `ownerGroup`, tous deux antérieurs à ce travail.)*
  - [x] **5.6.4** : Nœuds `ReviewCode` (systématique) + `ReviewSecurity`/`ReviewOps` (conditionnels au diff, exécution parallèle si tous deux déclenchés) après `RunDevcontainerTests`, avant `OpenPullRequest` — arête de synthèse `route_after_review` bouclant vers `DelegateToOpencode` (compteur dédié `review_attempts`, via un nœud `ReviewReconsideration` distinct de `AutoCorrectionLoop`, mêmes raisons qu'en 5.6.3). *(Fait le 2026-09-02 : fan-out natif LangGraph — `route_after_code_review` renvoie une liste de 0 à 2 clés, LangGraph exécute chacune en parallèle, convergence sur un nœud `ReviewGate` trivial (BSP natif, aucune synchronisation manuelle) avant la décision finale. `route_after_tests` inchangée (toujours "OpenPullRequest" en sortie) — c'est le `path_map` de `graph.py` qui la fait désormais atterrir sur `ReviewCode`, l'indirection existe précisément pour ce genre de changement. Suite verte (68 passed, 21 skipped avec la config habituelle de session ; 86 passed avec identifiants complets, mêmes deux échecs préexistants sans rapport qu'en 5.6.3).)*
  - [x] **5.6.5** : `AwaitHitlApproval` expose `outstanding_concerns` dans son payload d'interruption quand un budget de revue a été épuisé sans approbation (§4.5) — aucun passage en force ne doit rester invisible au relecteur humain. *(Fait le 2026-09-02 : `_outstanding_review_concerns`, sans flag dédié — le seul chemin qui laisse un verdict à `"request_changes"` dans l'état final EST un passage en force, un rejet avec budget restant étant toujours remplacé par un nouveau verdict avant de revenir ici. `interrupt()` n'étant pas testable hors runtime LangGraph réel, la logique est vérifiée directement sur la fonction pure. Suite verte : 70 passed, 21 skipped. Extension du schéma `pm_reviews`/Dashboard pour afficher ce champ hors périmètre de cette tâche — seul le payload du graphe était demandé.)*
...
* **Fichiers** : `services/pm-engine/pm_engine/{state,nodes,graph,deps,evidence_store}.py`, `pyproject.toml` (nouvelle dépendance `aioboto3`)
  - [x] **5.7.1** : `pm_engine/evidence_store.py` — client S3 minimal async (`upload_evidence(bucket, key, content) -> str`), vérifié contre l'instance S3/RustFS de dev réelle (objet réellement relu après téléversement, pas seulement écrit). *(Fait le 2026-09-02 : `aioboto3` (nouvelle dépendance, `uv.lock` régénéré), nouveau bucket `atelier-qa-evidence` (`deploy/dev/s3/dev-pod.yaml`/`README.md`). Round-trip binaire réel (256 octets non-UTF8) vérifié contre l'instance RustFS de dev. `S3_FORCE_PATH_STYLE` nécessaire pour RustFS (pas de DNS wildcard en dev, même contrainte déjà documentée côté `storage.rs` Rust).)*
  - [x] **5.7.2** : Nouveaux champs `PmEngineDeps` (`qa_workshop_devcontainer_repo`, `qa_evidence_s3: S3Config | None`) + nouveaux champs d'état (`QAVerdict`, `qa_verdict`, `qa_evidence_keys`). *(Fait le 2026-09-02 : `qa_evidence_s3` porte directement un `S3Config` construit une fois au démarrage (`s3_config_from_env`, `pm_engine.main`), plutôt qu'un nom de bucket séparé comme envisagé dans la spec — évite de dupliquer ce que `evidence_store.s3_config_from_env` sait déjà lire depuis l'environnement.)*
  - [x] **5.7.3** : Nœud `QAValidation` (provisionnement d'un Workshop dédié `pm-<issue>-qa` sur `main`, délégation OpenCode avec le contrat de sortie §5, récupération des preuves via `exec_in_workshop`/base64 — pas de nouvelle capacité MCP), câblé après `IndexKnowledge`, avant `FIN`. Jamais bloquant (§6) : toute erreur d'infrastructure dégrade vers un `qa_verdict` explicite, ne fait jamais échouer le graphe. *(Fait le 2026-09-02 : `_parse_qa_verdict` diffère volontairement de `_parse_review_verdict` — `opencode run` produit une transcription complète (raisonnement, appels d'outils), pas une réponse unique, donc extraction du DERNIER bloc JSON portant `"verdict"` plutôt que l'exigence stricte "la réponse entière EST le JSON" ; repli sur `"fail"`, jamais `"pass"`. 5 tests unitaires (transcription réaliste avec JSON non pertinent avant le verdict, réponse illisible, valeur de verdict inattendue). Suite verte : 77 passed, 22 skipped.)*
  - [x] **5.7.4** : Commentaire de PR post-merge résumant le verdict et les preuves (`BaseGitProvider.post_comment`). *(Fait le 2026-09-02, dans le cadre de 5.7.3 (`_finish_qa_validation`) — poste sur le numéro de PR s'il est connu (peut différer du numéro de ticket, observé en pratique), sinon sur le ticket, même repli que `merge_and_close`. Purement accessoire : un échec ne fait jamais échouer le nœud.)*
