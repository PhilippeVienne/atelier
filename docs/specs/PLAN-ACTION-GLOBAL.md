...
- [x] Le PM Engine résout un ticket de bout en bout de façon autonome. *(Validé le 2026-08-31 sur le ticket `todo-app#12`, run complet sans aucune intervention manuelle : 3 microVM Firecracker réelles, vrai Claude Code dans les guests, `AnalyzeIssue`→…→`OpenPullRequest`→`SuspendWhileWaitingReview`→interruption HITL. PR 13 : 12 fichiers, 941 lignes. Deux réserves honnêtes, qui relèvent de la qualité du résultat et non de la mécanique : (1) le découpage parallèle a produit du travail redondant — `api/` et `src/` implémentent deux fois la même fonctionnalité, chaque sous-tâche ayant recréé l'ensemble plutôt que sa part ; (2) `test_passed` valait `False` par un bug de l'api-server, corrigé depuis (`stream_is_done`, le flux SSE se fermait sans jamais émettre `exitCode`) mais pas encore re-exercé par un run complet.)*
