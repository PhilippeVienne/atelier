...
* **Fichiers** : `services/pm-engine/pm_engine/{state,nodes,graph,deps,evidence_store}.py`, `pyproject.toml` (nouvelle dépendance `aioboto3`)
  ...
  - [ ] **5.7.2** : Nouveaux champs `PmEngineDeps` (`qa_workshop_devcontainer_repo`, `qa_evidence_s3_bucket`, config S3) + nouveaux champs d'état (`qa_verdict`, `qa_evidence_keys`, §7.1 de la spec).
  - [ ] **5.7.3** : Nœud `QAValidation` (provisionnement d'un Workshop dédié `pm-<issue>-qa` sur `main`, délégation OpenCode avec le contrat de sortie §5, récupération des preuves via `exec_in_workshop`/base64 — pas de nouvelle capacité MCP), câblé après `IndexKnowledge`, avant `FIN`. Jamais bloquant (§6) : toute erreur d'infrastructure dégrade vers un `qa_verdict` explicite, ne fait jamais échouer le graphe.
  - [ ] **5.7.4** : Commentaire de PR post-merge résumant le verdict et les preuves (`BaseGitProvider.post_comment`).
