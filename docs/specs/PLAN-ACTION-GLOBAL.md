...
- [ ] Le PM Engine résout un ticket de bout en bout de façon autonome. **Le parcours complet a été exécuté pour de vrai** (2026-08-31, ticket `todo-app#9`) : vraies microVM Firecracker, vrai Claude Code dans le guest, `DelegateToClaudeCode`→`RunDevcontainerTests`→`OpenPullRequest`→suspension→interruption HITL. La PR produite contient du code réel (8 fichiers, 599 lignes, 10 tests verts dans le guest). Reste `[ ]` pour une seule raison : la dernière étape bloquante (`--permission-mode bypassPermissions`, sans quoi l'agent ne peut pas commiter) a été validée **à la main** dans le Workshop, puis portée dans `nodes.py` — le graphe lui-même n'a pas encore rejoué un ticket neuf avec ce correctif. Un run propre suffit à cocher.
