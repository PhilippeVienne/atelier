...
* **Fichiers** : `crates/cli` (nouveau), `crates/common/src/crd.rs` (`simulators`), `crates/controller/src/reconcile.rs`, `crates/mcp-gateway/src/`, `crates/api-server/src/approvals.rs` (nouveau), `dashboard/app/approvals/`.
  ...
  - [ ] **9.2** : Pont Tunnels dans la CLI — commandes `atelier ssh <id>` (PTY interactif via WS Axum), `atelier port-forward <id> <local>:<remote>`, et `atelier code <id>` (configuration dynamique SSH proxy command + lancement VS Code/Cursor remote).
