...
- **Deux services derriere le meme mot de passe n'acceptent pas la meme
  ...
  - **Suite (tache 12.6) : `atelier-api-server::routes::vscode_proxy_injects_real_session_auth_basic_header` est le test devenu perime par cette meme correction, jamais mis a jour** — reproduit et diagnostique de bout en bout contre l'infra reelle (OpenBao Kubernetes-auth active pour la premiere fois de cette session, cf. `deploy/dev/openbao/README.md`) : `GuestAuth::CodeServerCookie` (code actuel) joue un login POST `/login` PUIS la requete reelle sur la MEME connexion HTTP/1.1 garde-en-vie, mais le stub du test (`spawn_stub_http_server_capturing_authorization`) ne repond qu'a UNE seule requete par connexion (`Connection: close` code en dur) — la deuxieme requete echoue avec `hyper::Error` « operation was canceled », pas la cause que le nom du test suggere (« Basic header »), car ce nom et son assertion datent d'avant l'introduction de `CodeServerCookie`. Le test etait « vert » uniquement PARCE QUE le backend Kubernetes-auth d'OpenBao etait desactive dans cet environnement (`session_password` retourne alors `None`, `code_server_login` jamais appele, une seule requete emise) — l'activer a revele le vrai bug. Deja repere comme regression pre-existante sans lien (taches 9.3/12.5, confirme identique via `git stash`) : le corriger reste hors perimetre de 12.6 (dashboard), laisse a une suite — il faudra soit faire gerer plusieurs requetes/connexion au stub, soit faire asserter le cookie plutot que l'en-tete Basic.
...
- **Le CoW par `reflink` (`ioctl(FICLONE)`) echoue systematiquement sur ext4 (`[Errno 95] Operation not supported`).**
  - *Symptome observable* : Vouloir cloner instantanement un rootfs de VM sans duplication d'espace disque via `cp --reflink=always` ou l'appel systeme `ioctl(FICLONE)` renvoie une erreur systematique `EOPNOTSUPP (Operation not supported)`.
  - *Cause reelle* : ext4 ne supporte pas le clonage de blocs CoW au niveau filesystem (seuls btrfs, XFS et ZFS le supportent). Or les environnements standards Kubernetes (noeuds Kind sous Docker, volumes EBS gp3 AWS, disques persistants standards GCP) sont quasi-exclusivement formates en ext4.
  - *Solution validee empiriquement* : Firecracker autorise plusieurs microVMs simultanees a partager le **meme inode physique** de rootfs si le lecteur est configure en `is_read_only: true` (associe a `MovedResourceType::HardLinkedOrCopied`). Le boot jaile reel passe de plusieurs secondes de copie a **~20 ms**, a cout disque nul. Les ecritures in-guest doivent imperativement etre redirigees vers un disque ephemeral scratch ou un tmpfs assemble par OverlayFS cote guest.

- **`net-proxy` ne termine JAMAIS TLS sur son chemin de relais egress — une CA d'entreprise n'y "inspecte" donc rien (tache 11.1, spec 15).**
  - *Symptome observable/piege redactionnel* : la premiere redaction de `docs/specs/15-souverainete-airgap-inference-gpu.md` §3.2 affirmait que `net-proxy` "monte la ConfigMap `atelier-ca-bundle` et ajoute ces certificats a son client TLS sortant... peut ainsi inspecter et relayer le trafic HTTPS". Faux au vu du code reel : `crates/net-proxy/src/proxy.rs` (mode `CONNECT`, tunnel `TcpStream` brut) et `crates/net-proxy/src/tls_sni.rs` (mode transparent, lit uniquement le champ `server_name` en clair du `ClientHello` RFC 6066 pour router, sans jamais dechiffrer) ne terminent TLS nulle part sur ce chemin.
  - *Cause reelle* : `net-proxy` est concu comme un relai de bout en bout par design (voir le commentaire de module de `tls_sni.rs`) — la validation TLS reste entierement a la charge du client dans la microVM. Une CA d'entreprise cote `net-proxy` ne peut donc rien "inspecter" sans une reecriture MITM complete (hors perimetre, jamais implementee).
  - *Correction appliquee* : le point reel ou la confiance CA doit s'ajouter est la ou TLS est reellement termine — chaque client `reqwest` Rust du control plane (mecanisme generique `atelier_common::tls_client::client_builder_trusting_extra_ca`, `crates/common/src/tls_client.rs`) et l'outillage in-VM (`git`/`npm`/`pip`/`cargo`/`curl`, cote systeme d'exploitation, tache 11.2).

- **`reqwest::Certificate::from_pem` (backend `rustls-tls-native-roots`) accepte silencieusement un PEM invalide, sans jamais retourner d'erreur.**
  - *Symptome observable* : verifie empiriquement (`crates/common/src/tls_client.rs`, tests `garbage_bytes_are_rejected_explicitly`/`pem_block_with_corrupted_body_is_rejected_explicitly` avant correctif) : passer des octets totalement arbitraires, OU un bloc `-----BEGIN CERTIFICATE-----`/`-----END CERTIFICATE-----` dont le corps base64 est corrompu, a `reqwest::Certificate::from_pem(...)` puis `ClientBuilder::add_root_certificate(...).build()` reussit sans la moindre erreur a aucune des deux etapes.
  - *Cause reelle* : ni `from_pem` ni `build()` ne valident reellement la structure DER du certificat dans ce backend — l'echec ne surviendrait qu'au moment d'un vrai handshake TLS, bien plus tard et de facon opaque (`CERTIFICATE_VERIFY_FAILED` sans rapport evident avec la configuration fautive).
  - *Solution validee empiriquement* : decoder explicitement le PEM avec `rustls_pemfile::certs(&mut pem.as_slice())` (deja une dependance transitive de `rustls` dans ce workspace) et exiger qu'il en resulte au moins un certificat DER, AVANT de le transmettre a `reqwest::Certificate::from_pem`. Un CA operateur mal colle dans les values Helm echoue alors immediatement, avec un message explicite, au demarrage du composant concerne plutot qu'au premier appel reseau.

- **`WorkshopSpec.ownerSubject` doit contenir le `sub` JWT (un UUID Keycloak), jamais le `preferred_username` (tache 12.6).**
  - *Symptome observable* : des objets CRD `Workshop` de demo crees avec `ownerSubject: "demo"` ou `ownerSubject: "atelier-test-user"` (le nom d'utilisateur, intuitif mais faux) restent invisibles sur `/workshops` et `/campaigns` du Dashboard pour l'utilisateur correspondant — `list_workshops` (`crates/api-server/src/routes.rs`) filtre par `w.spec.owner_subject == user.subject`, et `user.subject` vient de `claims.sub` (`auth.rs`), un UUID opaque, jamais du nom affiche.
  - *Solution validee empiriquement* : recuperer le vrai `sub` via un password grant OIDC reel contre le Keycloak de dev (`curl .../protocol/openid-connect/token` puis decoder le JWT) avant de fabriquer un Workshop de demo appartenant a un utilisateur donne — `create_workshop` (cote normal, via l'UI ou le CLI) le fait deja correctement en le lisant du token de la requete, ce piege ne touche que la creation manuelle d'objets CRD pour des tests.
